	...
	methods: {
		loadDataList: function() {
			...
			let data = {
				...
				totalQuota: that.dataForm.totalQuota == '' ? null : that.dataForm.totalQuota,
				type: that.dataForm.type == '' ? null : that.dataForm.type,
				limitQuota: that.dataForm.limitQuota == '' ? null : that.dataForm.limitQuota,
				status: that.dataForm.status == '' ? null : that.dataForm.status,
				timeType: that.dataForm.timeType == '' ? null : that.dataForm.timeType
			};
			that.$http('voucher/searchVoucherByPage', 'POST', data, true, function(resp) {
				...
				for (let one of list) {
					...
					one.limitQuota = limitQuota[one.limitQuota + ''];
					one.discount = Math.floor(one.discount) + '元';
				}
				that.dataList = list;
		...
		updateHandle: function(id, status, uuid) {
			this.id = id;
			this.status = status;
			this.uuid = uuid;
			if (status == 1) {
		...
		confirmHandle: function() {
			...
			that.$http('voucher/updateVoucherStatus', 'POST', data, true, function(resp) {
		...
		deleteHandle: function(id) {
			...
			if (ids.length == 0) {
				...
				that.$confirm(`确定要删除选中的记录？`, '提示', {
					...
					that.$http('voucher/deleteVoucherByIds', 'POST', { ids: ids }, true, function(resp) {
						if (resp.rows > 0) {
							that.$message({
								message: '操作成功',
								type: 'success',
